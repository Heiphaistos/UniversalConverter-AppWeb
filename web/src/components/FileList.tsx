...
import { splitPdf, downloadBlob } from "../api";
...
function FileRow({ file, onUpdate, onConvert, onRemove }: RowProps) {
  ...
  async function handleSplit() {
    ...
    setSplitStatus("busy");
    try {
      const res = await splitPdf(file.file, pages);
      downloadBlob(res.blob, res.outputName);
      setSplitMsg(res.outputName);
      setSplitStatus("done");
    } catch (e: unknown) {
      setSplitMsg(e instanceof Error ? e.message : String(e));
      setSplitStatus("error");
    ...
    <div className={`rounded-xl transition-colors ${done ? "bg-slate-800/60 border border-green-900/40" : "bg-slate-800"}`}>
      ...
      {file.extension === "pdf" && file.pageCount && file.pageCount > 1 && !done && (
        <div className="px-4 pb-3 border-t border-slate-700/60">
          ...
          {splitStatus === "done" && splitMsg && (
            <p className="text-green-400 text-xs mt-1 truncate">✓ Téléchargé : {splitMsg}</p>
          )}
          {splitStatus === "error" && splitMsg && (
            <p className="text-red-400 text-xs mt-1">{splitMsg}</p>
          )}
        </div>
      )}

      {/* ── Résultat post-conversion ── */}
      {done && file.outputBlob && (
        <div className="px-4 pb-3 flex items-center gap-2 border-t border-green-900/20 pt-2">
          <div className="flex-1 min-w-0">
            <p className="text-green-400 text-xs truncate" title={file.outputName}>
              ✓ {file.outputName}
            </p>
