...
export function MergePromptModal({ fileIds, allFiles, onClose }: Props) {
  const pdfFiles = allFiles.filter(
    (f) => fileIds.includes(f.id) && f.status === "done" && f.outputBlob
  );
  ...
  async function merge() {
    ...
    try {
      ...
      const res = await mergePdfs(pdfFiles.map((f) => f.outputBlob!), mode, name);
          ...
          <div className="space-y-1">
            {pdfFiles.map((f, i) => (
              <div key={f.id} className="flex items-center gap-2 bg-slate-800/60 rounded-lg px-3 py-1.5">
                ...
                <span className="text-sm text-slate-300 truncate flex-1" title={f.outputName}>
                  {f.outputName}
                </span>
              </div>
            ))}
